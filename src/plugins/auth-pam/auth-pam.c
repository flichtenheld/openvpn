                       ...
                       struct openvpn_plugin_args_open_return *ret)
{
    ...
    if (pid)
    {
        ...
        {
            plugin_log(PLOG_ERR | PLOG_ERRNO,
                       MODULE,
                       ...
                       const char *envp[])
{
    ...
    if (type == OPENVPN_PLUGIN_AUTH_USER_PASS_VERIFY && context->foreground_fd >= 0)
    {
        ...
        if (username && strlen(username) > 0 && password)
        {
            ...
            {
                plugin_log(
                    PLOG_ERR | PLOG_ERRNO, MODULE, "Error sending auth info to background process");
            }
            else
            {
                     ...
                     const struct user_pass *up)
{
    ...
    plugin_log(PLOG_NOTE,
               MODULE,
               "BACKGROUND: deferred auth for '%s', pid=%d",
               up->username,
               (int)getpid());
    ...
    plugin_log(PLOG_NOTE,
               MODULE,
               "BACKGROUND: %s: deferred auth: PAM %s",
               up->username,
...
pam_server(int fd, const char *service, int verb, const struct name_value_list *name_value_list)
{
    ...
    {
        plugin_log(
            PLOG_ERR, MODULE, "BACKGROUND: could not load PAM lib %s: %s", pam_so, dlerror());
    ...
    while (1)
    {
        ...
        switch (command)
        {
            ...
            case -1:
                plugin_log(
                    PLOG_ERR | PLOG_ERRNO, MODULE, "BACKGROUND: read error on command channel");
                goto done;
