 ...
 **/
typedef DWORD (*devop_func_t)(_In_ HDEVINFO hDeviceInfoSet,
 ...
 **/
static DWORD
check_reboot(_In_ HDEVINFO hDeviceInfoSet,
             _In_ PSP_DEVINFO_DATA pDeviceInfoData,
             _Inout_ LPBOOL pbRebootRequired)
{
 ...
 **/
static DWORD
delete_device(_In_ HDEVINFO hDeviceInfoSet,
              _In_ PSP_DEVINFO_DATA pDeviceInfoData,
              _Inout_ LPBOOL pbRebootRequired)
{
 ...
 **/
static DWORD
change_device_state(_In_ HDEVINFO hDeviceInfoSet,
                    _In_ PSP_DEVINFO_DATA pDeviceInfoData,
                    _In_ BOOL bEnable,
                    _Inout_ LPBOOL pbRebootRequired)
{
    ...
    if (!SetupDiCallClassInstaller(DIF_PROPERTYCHANGE, hDeviceInfoSet, pDeviceInfoData))
    {
        ...
        msg(M_NONFATAL | M_ERRNO,
            "%s: SetupDiCallClassInstaller(DIF_PROPERTYCHANGE) failed",
            __FUNCTION__);
        return dwResult;
    }
 ...
 **/
static DWORD
enable_device(_In_ HDEVINFO hDeviceInfoSet,
              _In_ PSP_DEVINFO_DATA pDeviceInfoData,
              _Inout_ LPBOOL pbRebootRequired)
{
 ...
 **/
static DWORD
disable_device(_In_ HDEVINFO hDeviceInfoSet,
               _In_ PSP_DEVINFO_DATA pDeviceInfoData,
               _Inout_ LPBOOL pbRebootRequired)
{
...
get_reg_string(_In_ HKEY hKey, _In_ LPCWSTR szName, _Out_ LPWSTR *pszValue)
{
    ...
    if (dwResult != ERROR_SUCCESS)
    {
        ...
        msg(M_NONFATAL | M_ERRNO,
            "%s: enumerating \"%ls\" registry value failed",
            __FUNCTION__,
            szName);
        return dwResult;
    }
    ...
    switch (dwValueType)
    {
        ...
        case REG_EXPAND_SZ:
        {
            ...
            if (dwResult != ERROR_SUCCESS)
            {
                ...
                msg(M_NONFATAL | M_ERRNO,
                    "%s: reading \"%ls\" registry value failed",
                    __FUNCTION__,
                    szName);
                free(szValue);
                return dwResult;
            ...
            if (dwValueType == REG_EXPAND_SZ)
            {
                ...
                {
                    msg(M_NONFATAL | M_ERRNO,
                        "%s: expanding \"%ls\" registry value failed",
                        __FUNCTION__,
                        szName);
                    free(szValueExp);
                    free(szValue);
 ...
 **/
static DWORD
get_net_adapter_guid(_In_ HDEVINFO hDeviceInfoSet,
                     _In_ PSP_DEVINFO_DATA pDeviceInfoData,
                     ...
                     _Out_ LPGUID pguidAdapter)
{
    ...
    HKEY hKey = SetupDiOpenDevRegKey(
        hDeviceInfoSet, pDeviceInfoData, DICS_FLAG_GLOBAL, 0, DIREG_DRV, KEY_READ);
    ...
    while (iNumAttempts > 0)
    {
        ...
        if (dwResult != ERROR_SUCCESS)
        {
            ...
            msg(M_NONFATAL | M_ERRNO,
                "%s: querying \"NetCfgInstanceId\" registry value failed",
                __FUNCTION__);
            break;
        }
 ...
 **/
static DWORD
get_device_reg_property(_In_ HDEVINFO hDeviceInfoSet,
                        _In_ PSP_DEVINFO_DATA pDeviceInfoData,
                        ...
                        _Out_ LPVOID *ppData)
{
    ...
    if (SetupDiGetDeviceRegistryProperty(hDeviceInfoSet,
                                         pDeviceInfoData,
                                         dwProperty,
        ...
        if (dwResult == ERROR_INSUFFICIENT_BUFFER)
        {
            ...
            *ppData = malloc(dwRequiredSize);
            ...
            else
            {
                dwResult = GetLastError();
                msg(M_NONFATAL | M_ERRNO,
                    "%s: SetupDiGetDeviceRegistryProperty(%u) failed",
                    __FUNCTION__,
                    dwProperty);
                return dwResult;
            }
                   ...
                   _Out_ LPGUID pguidAdapter)
{
    ...
    if (!SetupDiCreateDeviceInfo(hDevInfoList,
                                 szClassName,
                                 &GUID_DEVCLASS_NET,
                                 szDeviceDescription,
                                 hwndParent,
    ...
    if (!SetupDiSetDeviceRegistryProperty(hDevInfoList,
                                          &devinfo_data,
                                          SPDRP_HARDWAREID,
                                          (const BYTE *)szHwId,
    ...
    typedef BOOL(WINAPI * DiInstallDeviceFn)(
        HWND, HDEVINFO, SP_DEVINFO_DATA *, SP_DRVINFO_DATA *, DWORD, BOOL *);
...
cleanup_remove_device:
    if (dwResult != ERROR_SUCCESS)
    {
        ...
        /* Set class installer parameters for DIF_REMOVE. */
        if (SetupDiSetClassInstallParams(hDevInfoList,
                                         &devinfo_data,
                                         ...
                                         sizeof(SP_REMOVEDEVICE_PARAMS)))
        {
            ...
            {
                msg(M_NONFATAL | M_ERRNO,
                    "%s: SetupDiCallClassInstaller(DIF_REMOVE) failed",
                    __FUNCTION__);
            }
        }
        else
 ...
 **/
static DWORD
execute_on_first_adapter(_In_opt_ HWND hwndParent,
                         ...
                         _Inout_ LPBOOL pbRebootRequired)
{
    ...
    HDEVINFO hDevInfoList = SetupDiGetClassDevsEx(
        &GUID_DEVCLASS_NET, NULL, hwndParent, DIGCF_PRESENT, NULL, NULL, NULL);
    if (hDevInfoList == INVALID_HANDLE_VALUE)
    {
        dwResult = GetLastError();
    ...
    for (DWORD dwIndex = 0;; dwIndex++)
    {
        ...
        if (!SetupDiEnumDeviceInfo(hDevInfoList, dwIndex, &devinfo_data))
        {
            ...
            else
            {
                ...
                msg(M_WARN | M_ERRNO,
                    "%s: SetupDiEnumDeviceInfo(%u) failed",
                    __FUNCTION__,
                    dwIndex);
                continue;
            }
...
tap_enable_adapter(_In_opt_ HWND hwndParent,
                   ...
                   _Inout_ LPBOOL pbRebootRequired)
{
    return execute_on_first_adapter(
        hwndParent, pguidAdapter, bEnable ? enable_device : disable_device, pbRebootRequired);
}
...
tap_set_adapter_name(_In_ LPCGUID pguidAdapter, _In_ LPCWSTR szName, _In_ BOOL bSilent)
{
    ...
    swprintf_s(
        szRegKey, _countof(szRegKey), szAdapterRegKeyPathTemplate, szDevClassNetId, szAdapterId);
                  ...
                  _Out_ struct tap_adapter_node **ppAdapter)
{
    ...
    *ppAdapter = NULL;
    ...
    for (DWORD dwIndex = 0;; dwIndex++)
    {
        ...
        dwResult = get_device_reg_property(hDevInfoList,
                                           &devinfo_data,
                                           SPDRP_HARDWAREID,
                                           &dwDataType,
        ...
        if (dwResult != ERROR_SUCCESS)
        {
            SetLastError(dwResult);
            msg(M_WARN | M_ERRNO,
                "%s: Cannot determine %ls adapter name",
                __FUNCTION__,
                szAdapterId);
            goto cleanup_hKey;
        }
