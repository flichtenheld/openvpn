...
bool
crypto_pem_encode(const char *name,
                  struct buffer *dst,
                  ...
                  struct gc_arena *gc)
{
    ...
    *dst = alloc_buf_gc(out_len, gc);
    if (!mbed_ok(mbedtls_pem_write_buffer(
            header, footer, BPTR(src), BLEN(src), BPTR(dst), BCAP(dst), &out_len))
...
rand_ctx_get(void)
{
    ...
    if (!rand_initialised)
    {
        ...
        if (!mbed_ok(mbedtls_ctr_drbg_seed(
                &cd_ctx, mbedtls_entropy_func, &ec, BPTR(&pers_string), BLEN(&pers_string))))
        {
...
void
cipher_ctx_init(mbedtls_cipher_context_t *ctx,
                const uint8_t *key,
...
int
cipher_ctx_update(
    mbedtls_cipher_context_t *ctx, uint8_t *dst, int *dst_len, uint8_t *src, int src_len)
{
...
cipher_ctx_final_check_tag(
    mbedtls_cipher_context_t *ctx, uint8_t *dst, int *dst_len, uint8_t *tag, size_t tag_len)
{
    size_t olen = 0;
...
ssl_tls1_PRF(const uint8_t *seed,
             ...
             int output_len)
{
    return mbed_ok(mbedtls_ssl_tls_prf(
        MBEDTLS_SSL_TLS_PRF_TLS1, secret, secret_len, "", seed, seed_len, output, output_len));
}
 ...
 */
static void
tls1_P_hash(const mbedtls_md_info_t *md_kt,
            const uint8_t *sec,
            int sec_len,
            const uint8_t *seed,
            int seed_len,
...
bool
ssl_tls1_PRF(
    const uint8_t *label, int label_len, const uint8_t *sec, int slen, uint8_t *out1, int olen)
{
    struct gc_arena gc = gc_new();
