                ...
                struct crypto_options **opt, bool floated, const uint8_t **ad_start)
{
    ...
    if (op != P_ACK_V1 && reliable_can_get(ks->rec_reliable))
    {
        ...
        if (reliable_ack_read_packet_id(buf, &id))
        {
             ...
             * always uses packet id 0. Ignore it if it claims another id:
             * accepting it would consume the receive window slot of that id
             * and make us drop the real packet with that id as a replay,
             * deadlocking the handshake. Dropping without acknowledging also
             * keeps the peer from purging that packet from its send window. */
