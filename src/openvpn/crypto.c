        ...
        if (ctx->cipher)
        {
            ...
            if (cipher_ctx_mode_cbc(ctx->cipher))
            {
                ...
                if (packet_id_initialized(&opt->packet_id)
                    && !packet_id_write(
                        &opt->packet_id.send, buf, opt->flags & CO_PACKET_ID_LONG_FORM, true))
                {
                    msg(D_CRYPT_ERRORS, "ENCRYPT ERROR: packet ID roll over");
                    goto err;
                }
 ...
 */
static bool
openvpn_decrypt_aead(struct buffer *buf,
                     struct buffer work,
 ...
 */
static bool
openvpn_decrypt_v1(struct buffer *buf,
                   struct buffer work,
                   struct crypto_options *opt,
                   const struct frame *frame)
{
        ...
        if (ctx->cipher)
        {
            ...
            /* Get packet ID from plaintext buffer or IV, depending on cipher mode */
            {
                if (cipher_ctx_mode_cbc(ctx->cipher))
                {
                    ...
                    {
                        if (!packet_id_read(
                                &pin, &work, BOOL_CAST(opt->flags & CO_PACKET_ID_LONG_FORM)))
                        {
...
bool
openvpn_decrypt(struct buffer *buf,
                struct buffer work,
                struct crypto_options *opt,
                const struct frame *frame,
                const uint8_t *ad_start)
...
warn_insecure_key_type(const char *ciphername)
{
    if (cipher_kt_insecure(ciphername))
    {
        msg(M_WARN,
            ...
            "OpenVPN 2.7.",
            ciphername,
            cipher_kt_block_size(ciphername) * 8);
    }
}
...
void
init_key_type(
    struct key_type *kt, const char *ciphername, const char *authname, bool tls_mode, bool warn)
{
...
void
init_key_ctx(struct key_ctx *ctx,
             ...
             const char *prefix)
{
    ...
    if (md_defined(kt->digest))
    {
        ...
        msg(D_CIPHER_INIT,
            ...
            prefix,
            md_kt_size(kt->digest) * 8,
            md_kt_name(kt->digest));
...
void
init_key_bi_ctx_send(struct key_ctx *ctx,
...
void
init_key_bi_ctx_recv(struct key_ctx *ctx,
                     const struct key_parameters *key_params,
                     const struct key_type *kt,
                     const char *name)
{
...
void
init_key_ctx_bi(struct key_ctx_bi *ctx,
...
check_tls_prf_working(void)
{
    ...
    int ret = ssl_tls1_PRF((uint8_t *)seed,
                           (int)strlen(seed),
                           (uint8_t *)secret,
                           (int)strlen(secret),
                           out,
