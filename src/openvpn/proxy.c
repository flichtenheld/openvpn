            ...
            if (!isprint(c) && !isspace(c)) /* not ascii? */
            {
                if (verbose)
                {
                    msg(D_LINK_ERRORS | M_ERRNO,
                        "recv_line: Non-ASCII character (%d) read on recv()",
                        (int)c);
                }
                *lookahead = la;
                return false;
                              ...
                              struct signal_info *sig_info)
{
    ...
    while (nparms >= 1 && status == 407)
    {
        ...
        else if (p->auth_method == HTTP_AUTH_NTLM2 && !processed) /* check for NTLM */
        {
            ...
            /* format HTTP CONNECT message */
            snprintf(
                buf, sizeof(buf), "CONNECT %s:%s HTTP/%s", host, port, p->options.http_version);
        ...
        else if (p->auth_method == HTTP_AUTH_DIGEST && !processed)
        {
            ...
            if (method == HTTP_AUTH_DIGEST)
            {
                ...
                {
                    msg(D_LINK_ERRORS,
                        "HTTP proxy: digest auth failed, malformed response "
                ...
                /* format HTTP CONNECT message */
                snprintf(
                    buf, sizeof(buf), "%s %s HTTP/%s", http_method, uri, p->options.http_version);
