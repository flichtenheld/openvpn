...
void
init_net_event_win32(struct rw_handle *event,
                     long network_events,
...
window_title_generate(const char *title)
{
    ...
    buf_printf(
        &out, "[%s] " PACKAGE_NAME " " PACKAGE_VERSION " F4:EXIT F1:USR1 F2:USR2 F3:HUP", title);
...
openvpn_execve(const struct argv *a, const struct env_set *es, const unsigned int flags)
{
    ...
    if (a && a->argv[0])
    {
        if (openvpn_execve_allowed(flags))
        {
            ...
            if (CreateProcessW(
                    cmd, cl, NULL, NULL, FALSE, proc_flags, env, NULL, &start_info, &proc_info))
            {
                DWORD exit_status = 0;
...
win_wfp_block_service(bool add, bool dns_only, int index, const HANDLE pipe)
{
    ...
    wfp_block_message_t data = {
        .header = { (add ? msg_add_wfp_block : msg_del_wfp_block), sizeof(wfp_block_message_t), 0 },
...
bool
send_msg_iservice(
    HANDLE pipe, const void *data, size_t size, ack_message_t *ack, const char *context)
{
    struct gc_arena gc = gc_new();
    DWORD len;
    bool ret = true;
