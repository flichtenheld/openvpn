           ...
           const struct route_list *rl)
{
    ...
    if (is_route_parm_defined(ro->metric))
    {
        ...
        {
            msg(M_WARN,
                PACKAGE_NAME " ROUTE: route metric for network %s (%s) must be >= 0",
                ro->network,
                ro->metric);
            goto fail;
        }
...
fail:
    msg(M_WARN,
        PACKAGE_NAME " ROUTE: failed to parse/resolve route for host/network: %s",
        ro->network);
    return false;
}
                ...
                openvpn_net_ctx_t *ctx)
{
    ...
    if (is_route_parm_defined(remote_endpoint))
    {
        ...
        rl->spec.remote_endpoint =
            getaddr(GETADDR_RESOLVE | GETADDR_HOST_ORDER | GETADDR_WARN_ON_SIGNAL,
                    remote_endpoint,
                    0,
                    &defined,
                    NULL);
        ...
        {
            msg(M_WARN,
                PACKAGE_NAME " ROUTE: failed to parse/resolve default gateway: %s",
                remote_endpoint);
            ret = false;
        }
    }
                     ...
                     openvpn_net_ctx_t *ctx)
{
    ...
    if (is_route_parm_defined(remote_endpoint))
    {
        ...
        {
            msg(M_WARN,
                PACKAGE_NAME " ROUTE: failed to parse/resolve VPN endpoint: %s",
                remote_endpoint);
            ret = false;
        }
    }
                  ...
                  openvpn_net_ctx_t *ctx)
{
    ...
    for (i = 0; i < rb->n_bypass; ++i)
    {
        ...
        {
            del_route3(
                rb->bypass[i], IPV4_NETMASK_HOST, gateway, tt, flags | ROUTE_REF_GW, rgi, es, ctx);
        }
    }
}
    ...
    if (rl && rl->flags & RG_ENABLE)
    {
        ...
        else
        {
            ...
            if (!local)
            {
                ...
                {
                    ret = add_route3(rl->spec.remote_host,
                                     IPV4_NETMASK_HOST,
                                     rl->rgi.gateway.addr,
                                     tt,
                                     flags | ROUTE_REF_GW,
            ...
            ret = add_bypass_routes(
                      &rl->spec.bypass, rl->rgi.gateway.addr, tt, flags, &rl->rgi, es, ctx)
                  && ret;
    ...
    if (rl && rl->iflags & RL_DID_REDIRECT_DEFAULT_GATEWAY)
    {
        ...
        if (rl->flags & RG_REROUTE_GW)
        {
            if (rl->flags & RG_DEF1)
            {
                ...
                del_route3(
                    0x00000000, 0x80000000, rl->spec.remote_endpoint, tt, flags, &rl->rgi, es, ctx);

                /* delete default route (2nd component) */
                del_route3(
                    0x80000000, 0x80000000, rl->spec.remote_endpoint, tt, flags, &rl->rgi, es, ctx);
            }
            else
            {
                /* delete default route */
                ...
                {
                    add_route3(
                        0, 0, rl->rgi.gateway.addr, tt, flags | ROUTE_REF_GW, &rl->rgi, es, ctx);
                }
            }
        }
...
bool
add_routes(struct route_list *rl,
    ...
    if (rl && !(rl->iflags & RL_ROUTES_ADDED))
    {
        ...
        if (management && rl->routes)
        {
            management_set_state(
                management, OPENVPN_STATE_ADD_ROUTES, NULL, NULL, NULL, NULL, NULL);
        }
#endif
          ...
          openvpn_net_ctx_t *ctx)
{
    ...
    status = RTA_SUCCESS;
    int ret = net_route_v4_add(
        ctx, &r->network, netmask_to_netbits2(r->netmask), &r->gateway, iface, 0, metric);
    if (ret == -EEXIST)
    {
    ...
    if (is_on_link(is_local_route, flags, rgi))
    {
        ...
        argv_printf_cat(
            &argv, "-cloning -net %s -netmask %s -interface %s", network, netmask, rgi->iface);
    }
    else
    {
        argv_printf_cat(&argv, "-net %s %s %s", network, gateway, netmask);
...
bool
add_route_ipv6(struct route_ipv6 *r6,
               const struct tuntap *tt,
               unsigned int flags,
               const struct env_set *es,
               openvpn_net_ctx_t *ctx)
{
    ...
    status = RTA_SUCCESS;
    int ret = net_route_v6_add(
        ctx, &r6->network, r6->netbits, gateway_needed ? &r6->gateway : NULL, device, 0, metric);
    if (ret == -EEXIST)
    {
        msg(D_ROUTE, "NOTE: Linux route add command failed because route exists");
        status = RTA_EEXIST;
    ...
    argv_printf(
        &argv, "%s add -inet6 %s -prefixlen %d %s", ROUTE_PATH, network, r6->netbits, gateway);
             ...
             openvpn_net_ctx_t *ctx)
{
    ...
    if (net_route_v4_del(
            ctx, &r->network, netmask_to_netbits2(r->netmask), &r->gateway, NULL, 0, metric)
        < 0)
    {
    ...
    else if ((flags & ROUTE_METHOD_MASK) == ROUTE_METHOD_ADAPTIVE)
    {
        ...
        if (!status)
        {
            ...
            openvpn_execve_check(
                &argv, es, 0, "ERROR: Windows route delete command failed [adaptive]");
            netcmd_semaphore_release();
        }
    }
    else
...
void
delete_route_ipv6(const struct route_ipv6 *r6,
                  const struct tuntap *tt,
                  const struct env_set *es,
                  openvpn_net_ctx_t *ctx)
{
    ...
    argv_printf(
        &argv, "%s delete -inet6 %s -prefixlen %d %s", ROUTE_PATH, network, r6->netbits, gateway);
...
do_route_ipv4_service(const bool add, const struct route_ipv4 *r, const struct tuntap *tt)
{
    ...
    route_message_t msg = {
        .header = { (add ? msg_add_route : msg_del_route), sizeof(route_message_t), 0 },
        .family = AF_INET,
