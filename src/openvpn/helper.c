...
print_opt_route(const in_addr_t network, const in_addr_t netmask, struct gc_arena *gc)
{
    ...
    {
        buf_printf(
            &out, "route %s %s", print_in_addr_t(network, 0, gc), print_in_addr_t(netmask, 0, gc));
    }
    else
    {
...
helper_client_server(struct options *o)
{
            ...
            if (o->topology == TOP_NET30 || o->topology == TOP_P2P)
            {
                ...
                if (!(o->server_flags & SF_NOPOOL))
                {
                    ...
                    ifconfig_pool_verify_range(
                        M_USAGE, o->ifconfig_pool_start, o->ifconfig_pool_end);
                }
                ...
                {
                    push_option(
                        o, print_opt_route(o->server_network, o->server_netmask, &o->gc), M_USAGE);
                }
    ...
    else if (o->server_bridge_defined || o->server_bridge_proxy_dhcp)
    {
        ...
        {
            verify_common_subnet("--server-bridge",
                                 o->server_bridge_ip,
                                 o->server_bridge_pool_start,
                                 o->server_bridge_netmask);
            verify_common_subnet("--server-bridge",
                                 o->server_bridge_pool_start,
                                 o->server_bridge_pool_end,
                                 o->server_bridge_netmask);
            verify_common_subnet("--server-bridge",
                                 o->server_bridge_ip,
                                 o->server_bridge_pool_end,
