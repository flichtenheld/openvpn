...
check_send_occ_load_test_dowork(struct context *c)
{
    if (connection_established(c))
    {
        ...
        {
            msg(M_INFO,
                "NOTE: failed to empirically measure MTU (requires " PACKAGE_NAME
...
check_send_occ_msg_dowork(struct context *c)
{
    ...
    switch (c->c2.occ_op)
    {
        ...
        case OCC_REPLY:
            ...
            if (!buf_write(
                    &c->c2.buf, c->c2.options_string_local, strlen(c->c2.options_string_local) + 1))
            {
                break;
            }
        ...
        case OCC_MTU_LOAD:
        {
            ...
            dmsg(D_PACKET_CONTENT,
                 "SENT OCC_MTU_LOAD min_int(%d,%d)-%d-%d-%d-%d) size=%d",
...
process_received_occ_msg(struct context *c)
{
    ...
    switch (buf_read_u8(&c->c2.buf))
    {
        ...
        case OCC_REPLY:
            ...
            {
                if (!options_cmp_equal_safe(
                        (char *)BPTR(&c->c2.buf), c->c2.options_string_remote, c->c2.buf.len))
                {
