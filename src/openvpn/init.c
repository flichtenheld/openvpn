            ...
            struct env_set *es)
{
    ...
    if (plugin_defined(plugins, plugin_type))
    {
        ...
        ASSERT(arg);
        argv_printf(
            &argv, "%s %d 0 %s %s %s", arg, tun_mtu, ifconfig_local, ifconfig_remote, context);
...
init_query_passwords(const struct context *c)
{
    ...
    if (c->options.auth_user_pass_file)
    {
        ...
        auth_user_pass_setup(
            c->options.auth_user_pass_file, c->options.auth_user_pass_file_inline, NULL);
#endif
    }
}
...
do_link_socket_addr_new(struct context *c)
{
    ALLOC_ARRAY_CLEAR_GC(
        c->c1.link_socket_addrs, struct link_socket_addr, c->c1.link_sockets_num, &c->gc);
}
        ...
        || options->show_tls_ciphers || options->show_curves)
    {
        ...
        if (options->show_tls_ciphers)
        {
            show_available_tls_ciphers(
                options->cipher_list, options->cipher_list_tls13, options->tls_cert_profile);
        }
...
do_genkey(const struct options *options)
{
    ...
    if (options->genkey && options->genkey_type == GENKEY_SECRET)
    {
        ...
        msg(D_GENKEY | M_NOPREFIX,
            "Randomly generated %d bit key written to %s",
            nbits_written,
    ...
    if (c0 && !c0->uid_gid_chroot_set)
    {
        ...
        if (c->options.selinux_context)
        {
            if (no_delay)
            {
                ...
                {
                    msg(M_ERR,
                        "setcon to '%s' failed; is /proc accessible?",
...
do_init_timers(struct context *c, bool deferred)
{
    ...
    if (!deferred)
    {
        ...
        {
            event_timeout_init(
                &c->c2.occ_mtu_load_test_interval, OCC_MTU_LOAD_INTERVAL_SECONDS, now);
        }
...
initialization_sequence_completed(struct context *c, const unsigned int flags)
{
    ...
    if (management)
    {
        ...
        management_set_state(
            management, OPENVPN_STATE_CONNECTED, detail, tun_local, tun_local6, &local, &remote);
        if (tun_local)
        {
...
do_open_tun(struct context *c, int *error_flags)
{
    ...
    *error_flags = 0;
    ...
    if (!can_preserve_tun(c->c1.tuntap))
    {
        ...
        {
            do_init_route_list(
                &c->options, c->c1.route_list, &c->c2.link_sockets[0]->info, c->c2.es, &c->net_ctx);
        }
        if (c->options.routes_ipv6 && c->c1.route_ipv6_list)
        {
            do_init_route_ipv6_list(&c->options,
                                    c->c1.route_ipv6_list,
                                    &c->c2.link_sockets[0]->info,
                                    c->c2.es,
                                    &c->net_ctx);
        }
        ...
        if (route_order(c->c1.tuntap) == ROUTE_BEFORE_TUN)
        {
            ...
            bool status = do_route(&c->options,
                                   c->c1.route_list,
                                   c->c1.route_ipv6_list,
                                   c->c1.tuntap,
                                   c->plugins,
        ...
        if (c->c2.did_open_tun)
        {
            ...
            if ((route_order(c->c1.tuntap) == ROUTE_AFTER_TUN) && c->options.route_delay_defined)
            {
                ...
                event_timeout_init(&c->c2.route_wakeup_expire,
                                   c->options.route_delay + c->options.route_delay_window,
                                   now);
    ...
    if (found & OPT_P_SOCKBUF)
    {
        ...
        for (int i = 0; i < c->c1.link_sockets_num; i++)
        {
            link_socket_update_buffer_sizes(
                c->c2.link_sockets[i], c->options.rcvbuf, c->options.sndbuf);
        }
    }
...
do_init_crypto_static(struct context *c, const unsigned int flags)
{
    ...
    packet_id_init(
        &c->c2.crypto_options.packet_id, options->replay_window, options->replay_time, "STATIC", 0);
    ...
    if (!key_ctx_bi_defined(&c->c1.ks.static_key))
    {
        ...
        init_key_type(
            &c->c1.ks.key_type, options->ciphername, options->authname, options->test_crypto, true);

        /* Read cipher and hmac keys from shared secret file */
        crypto_read_openvpn_key(&c->c1.ks.key_type,
                                &c->c1.ks.static_key,
                                options->shared_secret_file,
...
do_init_fragment(struct context *c)
{
    ...
    frame_calculate_dynamic(
        &c->c2.frame_fragment, &c->c1.ks.key_type, &c->options, get_link_socket_info(c));
...
do_compute_occ_strings(struct context *c)
{
    ...
    if (c->c2.tls_multi)
    {
        tls_multi_init_set_options(
            c->c2.tls_multi, c->c2.options_string_local, c->c2.options_string_remote);
    }
...
open_management(struct context *c)
{
    ...
    if (management)
    {
        if (c->options.management_addr)
        {
            ...
            {
                management_set_state(
                    management, OPENVPN_STATE_CONNECTING, NULL, NULL, NULL, NULL, NULL);
            }
...
init_instance(struct context *c, const struct env_set *env, const unsigned int flags)
{
    ...
    if (c->mode == CM_P2P || c->mode == CM_TOP || c->mode == CM_CHILD_TCP)
    {
        ...
        {
            frame_calculate_dynamic(
                &c->c2.frame, &c->c1.ks.key_type, &c->options, &c->c2.link_sockets[i]->info);
        }
    }
