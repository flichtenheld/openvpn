...
static int
ovpn_nl_msg_send(
    dco_context_t *dco, struct nl_msg *nl_msg, ovpn_nl_cb cb, void *cb_arg, const char *prefix)
{
    dco->status = 1;
             ...
             struct in6_addr *vpn_ipv6)
{
    ...
    if (vpn_ipv6)
    {
        NLA_PUT(nl_msg, OVPN_NEW_PEER_ATTR_IPV6, sizeof(struct in6_addr), vpn_ipv6);
    }
...
ovpn_dco_init_netlink(dco_context_t *dco)
{
    ...
    if (ret)
    {
        msg(M_ERR, "Cannot connect to generic netlink: %s", nl_geterror(ret));
    }
            ...
            const char *ciphername)
{
    ...
    if (dco_cipher != OVPN_CIPHER_ALG_NONE)
    {
        ...
        NLA_PUT(nl_msg, OVPN_KEY_DIR_ATTR_NONCE_TAIL, nonce_tail_len, encrypt_iv);
    }
    ...
    if (dco_cipher != OVPN_CIPHER_ALG_NONE)
    {
        ...
        NLA_PUT(nl_msg, OVPN_KEY_DIR_ATTR_NONCE_TAIL, nonce_tail_len, decrypt_iv);
    }
...
dco_parse_peer_multi(struct nl_msg *msg, void *arg)
{
    ...
    nla_parse(tb, OVPN_ATTR_MAX, genlmsg_attrdata(gnlh, 0), genlmsg_attrlen(gnlh, 0), NULL);
    ...
    {
        msg(M_WARN, "%s: cannot store DCO stats for peer %u", __func__, peer_id);
        return NL_SKIP;
    }
