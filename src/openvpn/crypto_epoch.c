...
void
ovpn_hkdf_expand(
    const uint8_t *secret, const uint8_t *info, int info_len, uint8_t *out, int out_len)
{
...
bool
ovpn_expand_label(const uint8_t *secret,
                  size_t secret_len,
                  const uint8_t *label,
                  size_t label_len,
...
epoch_key_iterate(struct epoch_key *epoch_key)
{
    ...
    ovpn_expand_label(epoch_key->epoch_key,
                      sizeof(epoch_key->epoch_key),
                      epoch_update_label,
                      epoch_update_label_len,
                      NULL,
                      0,
                      new_epoch_key.epoch_key,
                      ...
                      const struct key_type *kt)
{
    ...
    ovpn_expand_label(epoch_key->epoch_key,
                      sizeof(epoch_key->epoch_key),
                      epoch_data_key_label,
                      epoch_data_key_label_len,
                      NULL,
                      0,
                      (uint8_t *)(&key->cipher),
                      key->cipher_size);
...
epoch_generate_future_receive_keys(struct crypto_options *co)
{
    ...
    for (uint16_t i = co->epoch_data_keys_future_count - num_keys_generate;
         i < co->epoch_data_keys_future_count;
         i++)
    {
...
void
epoch_init_key_ctx(struct crypto_options *co,
...
epoch_check_send_iterate(struct crypto_options *opt)
{
    ...
    if (opt->aead_usage_limit)
    {
        if (aead_usage_limit_reached(
                opt->aead_usage_limit, &opt->key_ctx_bi.encrypt, opt->packet_id.send.id))
        {
         ...
         * */
                 ...
                 && (aead_usage_limit_reached(
                         opt->aead_usage_limit, &opt->key_ctx_bi.decrypt, opt->packet_id.rec.id)
