...
void
tls_crypt_init_key(struct key_ctx_bi *key,
                   struct key2 *keydata,
                   const char *key_file,
                   bool key_inline,
                   bool tls_server)
...
tls_session_generate_dynamic_tls_crypt_key(struct tls_session *session)
{
    ...
    init_key_ctx_bi(&session->tls_wrap_reneg.opt.key_ctx_bi,
                    &rengokeys,
                    key_direction,
                    &kt,
                    "dynamic tls-crypt");
...
tls_crypt_unwrap(const struct buffer *src, struct buffer *dst, struct crypto_options *opt)
{
    ...
    /* Decrypt cipher text */
    {
        ...
        if (!cipher_ctx_update(ctx->cipher,
                               BPTR(dst),
                               &outlen,
                               BPTR(src) + TLS_CRYPT_OFF_CT,
    ...
    /* Check authentication */
    {
        ...
        dmsg(
            D_PACKET_CONTENT, "TLS-CRYPT UNWRAP TO: %s", format_hex(BPTR(dst), BLEN(dst), 80, &gc));
...
void
tls_crypt_v2_init_client_key(struct key_ctx_bi *key,
...
void
tls_crypt_v2_init_server_key(struct key_ctx *key_ctx,
                             bool encrypt,
                             const char *key_file,
                             bool key_inline)
{
                             ...
                             struct gc_arena *gc)
{
    ...
    /* Encrypt */
    int outlen = 0;
    ASSERT(cipher_ctx_update(
        cipher_ctx, BEND(&work), &outlen, (void *)src_key->keys, sizeof(src_key->keys)));
    ASSERT(buf_inc_len(&work, outlen));
    ASSERT(cipher_ctx_update(
        cipher_ctx, BEND(&work), &outlen, BPTR(src_metadata), BLEN(src_metadata)));
    ASSERT(buf_inc_len(&work, outlen));
...
static bool
tls_crypt_v2_unwrap_client_key(struct key2 *client_key,
                               ...
                               struct key_ctx *server_key)
{
    ...
    dmsg(D_TLS_DEBUG_MED,
         "%s: unwrapping client key (len=%d): %s",
         __func__,
    ...
    int outlen = 0;
    if (!cipher_ctx_update(server_key->cipher,
                           BPTR(&plaintext),
                           &outlen,
    ...
    if (memcmp_constant_time(tag, tag_check, sizeof(tag_check)))
    {
        ...
        msg(D_TLS_DEBUG_LOW,
            "This might be a client-key that was generated for "
                                   ...
                                   bool server_key_inline)
{
    ...
    tls_crypt_v2_init_client_key(
        &test_client_key, &keydata, &test_wrapped_client_key, client_file, client_inline);
    ...
    ASSERT(tls_crypt_v2_unwrap_client_key(
        &test_client_key2, &test_metadata, test_wrapped_client_key, &server_key));
