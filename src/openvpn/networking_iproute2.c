...
net_addr_ll_set(openvpn_net_ctx_t *ctx, const openvpn_net_iface_t *iface, uint8_t *addr)
{
    ...
    argv_printf(
        &argv, "%s link set addr " MAC_FMT " dev %s", iproute_path, MAC_PRINT_ARG(addr), iface);
                    ...
                    const in_addr_t *remote)
{
    ...
    argv_printf(
        &argv, "%s addr add dev %s local %s peer %s", iproute_path, iface, local_str, remote_str);
    argv_msg(M_INFO, &argv);
    openvpn_execve_check(&argv, ctx->es, S_FATAL, "Linux ip addr add failed");
                    ...
                    const in_addr_t *remote)
{
    ...
    argv_printf(
        &argv, "%s addr del dev %s local %s peer %s", iproute_path, iface, local_str, remote_str);
    argv_msg(M_INFO, &argv);
    openvpn_execve_check(&argv, ctx->es, 0, "Linux ip addr del failed");
