 ...
 */
int xkey_native_sign(XKEY_KEYDATA *key,
                     unsigned char *sig,
                     size_t *siglen,
...
signature_settable_ctx_params(void *ctx, void *provctx)
{
    ...
    static OSSL_PARAM settable[] = { OSSL_PARAM_utf8_string(OSSL_SIGNATURE_PARAM_PAD_MODE, NULL, 0),
                                     OSSL_PARAM_utf8_string(OSSL_SIGNATURE_PARAM_DIGEST, NULL, 0),
                                     OSSL_PARAM_utf8_string(
                                         OSSL_SIGNATURE_PARAM_PSS_SALTLEN, NULL, 0),
                                     OSSL_PARAM_END };
...
static int
xkey_sign_dispatch(XKEY_SIGNATURE_CTX *sctx,
                   unsigned char *sig,
                   size_t *siglen,
                   const unsigned char *tbs,
                   size_t tbslen)
...
static int
signature_digest_verify_init(void *ctx,
                             const char *mdname,
                             void *provkey,
...
static int
signature_digest_sign(void *ctx,
                      unsigned char *sig,
                      size_t *siglen,
                      size_t sigsize,
                      const unsigned char *tbs,
 ...
 */
int
xkey_native_sign(XKEY_KEYDATA *key,
                 unsigned char *sig,
                 size_t *siglen,
                 const unsigned char *tbs,
                 size_t tbslen,
    ...
    if (EVP_PKEY_get_id(pkey) == EVP_PKEY_RSA)
    {
        ...
        if (!strcmp(sigalg.padmode, "pss"))
        {
            params[i++] = OSSL_PARAM_construct_utf8_string(
                OSSL_SIGNATURE_PARAM_PSS_SALTLEN, (char *)saltlen, 0);
            /* same digest for mgf1 */
            params[i++] = OSSL_PARAM_construct_utf8_string(
                OSSL_SIGNATURE_PARAM_MGF1_DIGEST, (char *)mdname, 0);
        }
    }
...
int
xkey_provider_init(const OSSL_CORE_HANDLE *handle,
