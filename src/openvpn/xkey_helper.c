...
int
xkey_digest(
    const unsigned char *src, size_t srclen, unsigned char *buf, size_t *buflen, const char *mdname)
{
 ...
 */
EVP_PKEY *
xkey_load_generic_key(OSSL_LIB_CTX *libctx,
                      void *handle,
                      EVP_PKEY *pubkey,
 ...
 */
int
xkey_management_sign(void *unused,
                     unsigned char *sig,
                     size_t *siglen,
 ...
 */
bool
encode_pkcs1(unsigned char *enc,
             size_t *enc_len,
             const char *mdname,
             const unsigned char *tbs,
             size_t tbslen)
