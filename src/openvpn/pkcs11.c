...
static pkcs11h_engine_system_t s_pkcs11h_sys_engine = { malloc,
                                                        free,
                                                        __mytime,
                                                        __mysleep,
#if defined(_WIN32)
...
static void
_pkcs11_openvpn_log(void *const global_data,
                    unsigned flags,
...
static PKCS11H_BOOL
_pkcs11_openvpn_token_prompt(void *const global_data,
                             ...
                             const unsigned retry)
{
    ...
    snprintf(
        token_resp.username, sizeof(token_resp.username), "Please insert %s token", token->label);

    if (!get_user_pass(&token_resp,
                       NULL,
...
static PKCS11H_BOOL
_pkcs11_openvpn_pin_prompt(void *const global_data,
                           void *const user_data,
                           const pkcs11h_token_id_t token,
                           const unsigned retry,
                           char *const pin,
...
pkcs11_initialize(const bool protected_auth, const int nPINCachePeriod)
{
    ...
    {
        msg(M_FATAL,
            "PKCS#11: Cannot initialize system engine %ld-'%s'",
            rv,
    ...
    {
        msg(M_FATAL,
            "PKCS#11: Cannot set protected authentication mode %ld-'%s'",
            rv,
            pkcs11h_getMessage(rv));
        goto cleanup;
    }
...
cleanup:
    dmsg(
        D_PKCS11_DEBUG, "PKCS#11: pkcs11_initialize - return %ld-'%s'", rv, pkcs11h_getMessage(rv));
...
bool
pkcs11_addProvider(const char *const provider,
                   ...
                   const bool cert_private)
{
    ...
    {
        msg(M_WARN,
            "PKCS#11: Cannot register provider '%s' %ld-'%s'",
            provider,
            rv,
            pkcs11h_getMessage(rv));
    }
    else
    {
        ...
        rv = pkcs11h_setProviderProperty(
            provider, PKCS11H_PROVIDER_PROPERTY_LOCATION, provider, strlen(provider) + 1);
        ...
        if (rv == CKR_OK && platform_absolute_pathname(provider))
        {
            ...
            rv = pkcs11h_setProviderProperty(provider,
                                             PKCS11H_PROVIDER_PROPERTY_LOADER_FLAGS,
                                             &loader_flags,
                                             sizeof(loader_flags));
        }
#endif
        ...
        {
            msg(M_WARN,
                "PKCS#11: Cannot initialize provider '%s' %ld-'%s'",
                provider,
                rv,
                pkcs11h_getMessage(rv));
    ...
    if ((rv = pkcs11h_addProvider(provider,
                                  provider,
                                  protected_auth,
                                  private_mode,
...
pkcs11_management_id_count(void)
{
    ...
    if ((rv = pkcs11h_certificate_enumCertificateIds(
             PKCS11H_ENUM_METHOD_CACHE_EXIST, NULL, PKCS11H_PROMPT_MASK_ALLOW_ALL, NULL, &id_list))
        != CKR_OK)
    {
...
pkcs11_management_id_get(const int index, char **id, char **base64)
{
    ...
    *base64 = NULL;
    ...
    {
        msg(M_WARN,
            "PKCS#11: Cannot serialize certificate id %ld-'%s'",
            rv,
            pkcs11h_getMessage(rv));
        goto cleanup;
    }
    ...
    if ((rv = pkcs11h_certificate_create(entry->certificate_id,
                                         NULL,
    ...
    if ((rv = pkcs11h_certificate_getCertificateBlob(
             certificate, certificate_blob, &certificate_blob_size))
        != CKR_OK)
    {
        msg(M_WARN, "PKCS#11: Cannot get certificate blob %ld-'%s'", rv, pkcs11h_getMessage(rv));
        goto cleanup;
...
int
tls_ctx_use_pkcs11(struct tls_root_ctx *const ssl_ctx,
                   ...
                   const char *const pkcs11_id)
{
    ...
    if ((rv = pkcs11h_certificate_create(certificate_id,
                                         NULL,
                                         PKCS11H_PROMPT_MASK_ALLOW_ALL,
                                         PKCS11H_PIN_CACHE_INFINITE,
                                         &certificate))
...
static PKCS11H_BOOL
_pkcs11_openvpn_show_pkcs11_ids_pin_prompt(void *const global_data,
                                           void *const user_data,
                                           const pkcs11h_token_id_t token,
                                           const unsigned retry,
                                           char *const pin,
...
show_pkcs11_ids(const char *const provider, bool cert_private)
{
    ...
    {
        msg(M_FATAL,
            "PKCS#11: Cannot set protected authentication %ld-'%s'",
            rv,
            pkcs11h_getMessage(rv));
        goto cleanup;
    }
    ...
    if ((rv = pkcs11h_certificate_enumCertificateIds(PKCS11H_ENUM_METHOD_CACHE_EXIST,
                                                     NULL,
                                                     PKCS11H_PROMPT_MASK_ALLOW_ALL,
                                                     NULL,
                                                     &user_certificates))
    ...
    for (current = user_certificates; current != NULL; current = current->next)
    {
        ...
        if ((rv = pkcs11h_certificate_serializeCertificateId(
                 NULL, &ser_len, current->certificate_id))
            != CKR_OK)
        {
            msg(M_FATAL,
                "PKCS#11: Cannot serialize certificate %ld-'%s'",
                rv,
                pkcs11h_getMessage(rv));
            goto cleanup1;
        }
