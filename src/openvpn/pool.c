...
ifconfig_pool_init(const bool ipv4_pool,
                   enum pool_type type,
                   in_addr_t start,
                   in_addr_t end,
                   ...
                   const int ipv6_netbits)
{
    ...
    if (pool->ipv4.enabled)
    {
        ...
        msg(D_IFCONFIG_POOL,
            "IFCONFIG POOL IPv4: base=%s size=%d",
    ...
    if (pool->ipv6.enabled)
    {
        ...
        {
            msg(D_IFCONFIG_POOL,
                "IFCONFIG POOL IPv6: incrementing pool start "
        ...
        msg(D_IFCONFIG_POOL,
            "IFCONFIG POOL IPv6: base=%s size=%d netbits=%d",
...
static void
ifconfig_pool_set(struct ifconfig_pool *pool,
                  const char *cn,
