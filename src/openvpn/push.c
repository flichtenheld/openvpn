 ...
 */
void
server_pushed_signal(struct context *c,
...
parse_auth_pending_keywords(const struct buffer *buffer, unsigned int *server_timeout)
{
    ...
    if (!buf_advance(&buf, strlen("AUTH_PENDING")) || !(buf_read_u8(&buf) == ',') || !BLEN(&buf))
    {
        ...
        if (management)
        {
            management_set_state(
                management, OPENVPN_STATE_AUTH_PENDING, "", NULL, NULL, NULL, NULL);
        }
#endif
    ...
    {
        management_set_state(
            management, OPENVPN_STATE_AUTH_PENDING, BSTR(&buf), NULL, NULL, NULL, NULL);
    }
#endif
 ...
 */
static bool push_option_fmt(
    struct gc_arena *gc, struct push_list *push_list, int msglevel, const char *fmt, ...)
#ifdef __GNUC__
...
send_auth_failed(struct context *c, const char *client_reason)
{
...

    {
        ...
        send_control_channel_string_dowork(
            &c->c2.tls_multi->session[TM_INITIAL], BSTR(&buf), D_PUSH);
        send_control_channel_string_dowork(
            &c->c2.tls_multi->session[TM_ACTIVE], BSTR(&buf), D_PUSH);
...
incoming_push_message(struct context *c, const struct buffer *buffer)
{
    ...
    msg(D_PUSH,
        "PUSH: Received control message: '%s'",
        sanitize_control_message(BSTR(buffer), &gc));

    int status = process_incoming_push_msg(
        c, buffer, c->options.pull, pull_permission_mask(c), &option_types_found);

    if (status == PUSH_MSG_ERROR)
    {
        msg(D_PUSH_ERRORS,
            "WARNING: Received bad push/pull message: %s",
            sanitize_control_message(BSTR(buffer), &gc));
    }
...
send_push_request(struct context *c)
{
    ...
    {
        msg(D_STREAM_ERRORS,
            "No reply from server to push requests in %ds",
 ...
 */
void
prepare_auth_token_push_reply(struct tls_multi *tls_multi,
                              struct gc_arena *gc,
                              struct push_list *push_list)
{
    ...
    if (tls_multi->auth_token)
    {
        ...
        int ret = openvpn_base64_encode(
            tls_multi->locked_username, (int)strlen(tls_multi->locked_username), &base64user);
...
static bool
send_push_options(struct context *c,
                  struct buffer *buf,
                  struct push_list *push_list,
                  int safe_cap,
                  bool *push_sent,
...
static void
push_option_ex(
    struct gc_arena *gc, struct push_list *push_list, const char *opt, bool enable, int msglevel)
{
...
static bool
push_option_fmt(
    struct gc_arena *gc, struct push_list *push_list, int msglevel, const char *format, ...)
{
    va_list arglist;
    char tmp[256] = { 0 };
    int len;
...
remove_iroutes_from_push_route_list(struct options *o)
{
    if (o && o->push_list.head && (o->iroutes || o->iroutes_ipv6))
    {
        ...
        while (e)
        {
            ...
            if (e->enable
                && parse_line(
                    e->option, p, SIZE(p) - 1, "[PUSH_ROUTE_REMOVE]", 1, D_ROUTE_DEBUG, &gc))
            {
