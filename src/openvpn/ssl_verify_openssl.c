...
verify_callback(int preverify_ok, X509_STORE_CTX *ctx)
{
    ...
    if (!preverify_ok && !session->opt->verify_hash_no_ca)
    {
        ...
        msg(D_TLS_ERRORS,
            "VERIFY ERROR: depth=%d, error=%s: %s, serial=%s",
            X509_STORE_CTX_get_error_depth(ctx),
            X509_verify_cert_error_string(X509_STORE_CTX_get_error(ctx)),
            subject,
    ...
    if (extensions)
    {
        ...
        for (i = 0; i < numalts; i++)
        {
            ...
            switch (name->type)
            {
                ...
                default:
                    msg(D_TLS_DEBUG,
                        "%s: ignoring general name field type %i",
                        __func__,
                        name->type);
                    break;
            }
    ...
    else if (strcmp(LN_serialNumber, x509_username_field) == 0)
    {
        ...
        if (FAILURE
            == extract_x509_field_ssl(
                X509_get_subject_name(peer_cert), x509_username_field, common_name, cn_len))
        {
            return FAILURE;
        }
...
x509_get_subject(X509 *cert, struct gc_arena *gc)
{
    ...
    X509_NAME_print_ex(subject_bio,
