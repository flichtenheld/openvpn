...
crypto_print_openssl_errors(const unsigned int flags)
{
    ...
    while ((err = ERR_get_error_all(&file, &line, &func, &data, &errflags)) != 0)
    {
        ...
        {
            msg(D_CRYPT_ERRORS,
                "TLS error: The server has no TLS ciphersuites "
...
show_available_ciphers(void)
{
    ...
    qsort((EVP_CIPHER *)cipher_list.list,
          cipher_list.num,
          sizeof(*cipher_list.list),
          cipher_name_cmp);
...
{
    printf(
        "%s %d bit digest size\n", md_kt_name(EVP_MD_get0_name(digest)), EVP_MD_size(digest) * 8);
}
...
void
cipher_ctx_init(EVP_CIPHER_CTX *ctx,
                const uint8_t *key,
...
cipher_ctx_final_check_tag(
    EVP_CIPHER_CTX *ctx, uint8_t *dst, int *dst_len, uint8_t *tag, size_t tag_len)
{
             ...
             int output_len)
{
    ...
    params[1] = OSSL_PARAM_construct_octet_string(
        OSSL_KDF_PARAM_SECRET, (uint8_t *)secret, (size_t)secret_len);
    params[2] =
    ...
    {
        crypto_msg(D_TLS_DEBUG_LOW,
                   "Generating TLS 1.0 PRF using "
