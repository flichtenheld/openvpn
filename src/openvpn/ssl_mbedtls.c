    ...
    struct tls_session *session, const char *label, size_t label_size, void *ekm, size_t ekm_size)
{
    ...
    int ret = mbedtls_ssl_tls_prf(cache->tls_prf_type,
                                  cache->master_secret,
                                  sizeof(cache->master_secret),
                                  label,
                                  cache->client_server_random,
...
tls_ctx_load_dh_params(struct tls_root_ctx *ctx, const char *dh_file, bool dh_inline)
{
    if (dh_inline)
    {
        if (!mbed_ok(mbedtls_dhm_parse_dhm(
                ctx->dhm_ctx, (const unsigned char *)dh_file, strlen(dh_file) + 1)))
        {
    ...
    msg(D_TLS_DEBUG_LOW,
        "Diffie-Hellman initialized with " counter_format " bit key",
...
int
tls_ctx_load_pkcs12(struct tls_root_ctx *ctx,
...
tls_ctx_load_cert_file(struct tls_root_ctx *ctx, const char *cert_file, bool cert_inline)
{
    ...
    if (cert_inline)
    {
        if (!mbed_ok(mbedtls_x509_crt_parse(
                ctx->crt_chain, (const unsigned char *)cert_file, strlen(cert_file) + 1)))
        {
...
tls_ctx_load_priv_file(struct tls_root_ctx *ctx, const char *priv_key_file, bool priv_key_inline)
{
    ...
    {
        status = mbedtls_compat_pk_parse_keyfile(
            ctx->priv_key, priv_key_file, NULL, mbedtls_ctr_drbg_random, rand_ctx_get());
        if (MBEDTLS_ERR_PK_PASSWORD_REQUIRED == status)
        {
            char passbuf[512] = { 0 };
            pem_password_callback(passbuf, 512, 0, NULL);
            status = mbedtls_compat_pk_parse_keyfile(
                ctx->priv_key, priv_key_file, passbuf, mbedtls_ctr_drbg_random, rand_ctx_get());
        }
    }
    if (!mbed_ok(status))
    {
    ...
    if (!mbed_ok(mbedtls_compat_pk_check_pair(
            &ctx->crt_chain->pk, ctx->priv_key, mbedtls_ctr_drbg_random, rand_ctx_get())))
    {
                                  ...
                                  void *sign_ctx)
{
    ...
    ALLOC_OBJ_CLEAR(ctx->priv_key, mbedtls_pk_context);
    if (!mbed_ok(mbedtls_pk_setup_rsa_alt(
            ctx->priv_key, &ctx->external_key, NULL, external_pkcs1_sign, external_key_len)))
    {
        return 1;
    }
...
void
tls_ctx_load_ca(struct tls_root_ctx *ctx,
                const char *ca_file,
                bool ca_inline,
                const char *ca_path,
                bool tls_server)
{
    ...
    {
        if (!mbed_ok(mbedtls_x509_crt_parse(
                ctx->ca_chain, (const unsigned char *)ca_file, strlen(ca_file) + 1)))
        {
...
void
tls_ctx_load_extra_certs(struct tls_root_ctx *ctx,
...
backend_tls_ctx_reload_crl(struct tls_root_ctx *ctx, const char *crl_file, bool crl_inline)
{
    ...
    if (crl_inline)
    {
        if (!mbed_ok(mbedtls_x509_crl_parse(
                ctx->crl, (const unsigned char *)crl_file, strlen(crl_file) + 1)))
        {
                   ...
                   struct tls_session *session)
{
    ...
    mbedtls_ssl_config_defaults(ks_ssl->ssl_config,
                                ssl_ctx->endpoint,
...
key_state_ssl_shutdown(struct key_state_ssl *ks_ssl)
{
    mbedtls_ssl_send_alert_message(
        ks_ssl->ctx, MBEDTLS_SSL_ALERT_LEVEL_FATAL, MBEDTLS_SSL_ALERT_MSG_CLOSE_NOTIFY);
}
