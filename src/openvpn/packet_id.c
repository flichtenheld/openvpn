...
static void
packet_id_init_recv(
    struct packet_id_rec *rec, int seq_backtrack, int time_backtrack, const char *name, int unit)
{
    rec->name = name;
    rec->unit = unit;
    if (seq_backtrack)
...
void
packet_id_init(
    struct packet_id *p, int seq_backtrack, int time_backtrack, const char *name, int unit)
{
    dmsg(D_PID_DEBUG,
    ...
    if (p->seq_backtrack)
    {
        ...
        if (pin->time == p->time)
        {
            ...
            if (diff > p->max_backtrack_stat)
            {
                ...
                packet_id_debug(D_PID_DEBUG_LOW,
                                p,
                                pin,
