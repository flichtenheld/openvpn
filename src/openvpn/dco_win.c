...
dco_get_version(OVPN_VERSION *version)
{
    ...
    if (!DeviceIoControl(
            h, OVPN_IOCTL_GET_VERSION, NULL, 0, version, sizeof(*version), &bytes_returned, NULL))
    {
        goto done;
    }
...
ovpn_dco_init_mp(dco_context_t *dco, const char *dev_node)
{
    ...
    DWORD bytes_returned = 0;
    if (!DeviceIoControl(
            dco->tt->hand, OVPN_IOCTL_SET_MODE, &m, sizeof(m), NULL, 0, &bytes_returned, NULL))
    {
...
dco_p2p_new_peer(HANDLE handle, struct link_socket *sock, struct signal_info *sig_info)
{
    ...
    if (!DeviceIoControl(handle, OVPN_IOCTL_NEW_PEER, &peer, sizeof(peer), NULL, 0, NULL, &ov))
    {
        ...
        {
            dco_connect_wait(
                handle, &ov, get_server_poll_remaining_time(sock->server_poll_timeout), sig_info);
        }
    }
}

int
dco_new_peer(dco_context_t *dco,
             unsigned int peerid,
             int sd,
             ...
             struct in6_addr *vpn_ipv6)
{
    ...
    DWORD bytesReturned;
    if (!DeviceIoControl(dco->tt->hand,
...
{
    msg(D_DCO_DEBUG,
        "%s: peer-id %d, keepalive %d/%d, mss %d",
        __func__,
        peerid,
        keepalive_interval,
        keepalive_timeout,
...
{
    msg(D_DCO_DEBUG,
        "%s: slot %d, key-id %d, peer-id %d, cipher %s",
        __func__,
        slot,
        keyid,
        peerid,
...
dco_get_peer_stats(struct context *c, const bool raise_sigusr1_on_err)
{
    ...
    DWORD bytes_returned = 0;
    if (!DeviceIoControl(
            tt->hand, OVPN_IOCTL_GET_STATS, NULL, 0, &stats, sizeof(stats), &bytes_returned, NULL))
    {
...
void
dco_win_add_iroute_ipv4(dco_context_t *dco,
                        in_addr_t dst,
                        ...
                        unsigned int peer_id)
{
    ...
    msg(D_DCO_DEBUG,
        "%s: %s/%d -> peer %d",
        __func__,
        print_in_addr_t(dst, IA_NET_ORDER, &gc),
        netbits,
        peer_id);
...
void
dco_win_add_iroute_ipv6(dco_context_t *dco,
                        struct in6_addr dst,
                        unsigned int netbits,
                        unsigned int peer_id)
{
    ...
    msg(D_DCO_DEBUG,
        "%s: %s/%d -> peer %d",
        __func__,
        print_in6_addr(dst, IA_NET_ORDER, &gc),
        netbits,
        peer_id);
