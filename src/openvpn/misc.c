...
static bool
auth_user_pass_mgmt(struct user_pass *up,
                    const char *prefix,
    ...
    if (!up->defined)
    {
            ...
            && management_query_user_pass_enabled(management))
        {
            ...
            if (flags & GET_USER_PASS_NEED_OK)
            {
                ...
                if (!query_user_SINGLE(
                        BSTR(&user_prompt), BLEN(&user_prompt), up->password, USER_PASS_LEN, false))
                {
        ...
        if (username_from_stdin || password_from_stdin || response_from_stdin)
        {
...
#endif /* ifdef ENABLE_MANAGEMENT */
            {
                ...
                {
                    query_user_add(
                        BSTR(&user_prompt), BLEN(&user_prompt), up->username, USER_PASS_LEN, true);
                }

                if (password_from_stdin)
                {
                    query_user_add(
                        BSTR(&pass_prompt), BLEN(&pass_prompt), up->password, USER_PASS_LEN, false);
                }
