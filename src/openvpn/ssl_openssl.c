...
tls_ctx_server_new(struct tls_root_ctx *ctx)
{
    ...
    {
        crypto_msg(M_WARN,
                   "Warning: TLS server context initialisation "
                   "has warnings.");
    }
}
...
tls_ctx_client_new(struct tls_root_ctx *ctx)
{
    ...
    {
        crypto_msg(M_WARN,
                   "Warning: TLS client context initialisation "
                   "has warnings.");
    }
}
    ...
    for (; begin_of_cipher < strlen(ciphers); begin_of_cipher = end_of_cipher)
    {
        ...
        else
        {
            ...
            if (end_of_cipher - begin_of_cipher == current_cipher_len
                && 0
                       != memcmp(&ciphers[begin_of_cipher],
...
tls_ctx_load_dh_params(struct tls_root_ctx *ctx, const char *dh_file, bool dh_file_inline)
{
    ...
    {
        crypto_msg(M_FATAL,
                   "Cannot load DH parameters from %s",
...
ui_reader(UI *ui, UI_STRING *uis)
{
    ...
    if (UI_get_string_type(uis) == UIT_PROMPT)
    {
        ...
        if (strstr(prompt, "PKCS#11"))
        {
            ...
            CLEAR(up);
            get_user_pass(
                &up, NULL, "PKCS#11 token", GET_USER_PASS_MANAGEMENT | GET_USER_PASS_PASSWORD_ONLY);
                    ...
                    bool load_ca_file)
{
    ...
    if (!PKCS12_parse(p12, "", &pkey, &cert, &ca))
    {
        ...
        {
            crypto_msg(M_WARN,
                       "Decoding PKCS12 failed. Probably wrong password "
...
backend_tls_ctx_reload_crl(struct tls_root_ctx *ssl_ctx, const char *crl_file, bool crl_inline)
{
    ...
    while (true)
    {
        ...
        if (crl == NULL)
        {
            ...
            crypto_msg(M_WARN,
                       "CRL: cannot read CRL from file %s",
        ...
        if (!X509_STORE_add_crl(store, crl))
        {
            X509_CRL_free(crl);
            crypto_msg(
                M_WARN, "CRL: cannot add %s to store", print_key_filename(crl_file, crl_inline));
            break;
        }
        X509_CRL_free(crl);
        num_crls_loaded++;
 ...
 */
static int
get_sig_from_man(const unsigned char *dgst,
...
static ECDSA_SIG *
ecdsa_sign_sig(
    const unsigned char *dgst, int dgstlen, const BIGNUM *in_kinv, const BIGNUM *in_r, EC_KEY *ec)
{
                         ...
                         bool extra_certs_file_inline)
{
    ...
    {
        crypto_msg(M_FATAL,
                   "Cannot load extra-certs file: %s",
...
bio_debug_data(const char *mode, BIO *bio, const uint8_t *buf, int len, const char *desc)
{
    ...
    if (len > 0)
    {
        open_biofp();
        fprintf(biofp,
                "BIO_%s %s time=%" PRIi64 " bio=" ptr_format " len=%d data=%s\n",
                mode,
                desc,
                (int64_t)time(NULL),
                (ptr_type)bio,
...
bio_debug_oc(const char *mode, BIO *bio)
{
    open_biofp();
    fprintf(biofp,
            "BIO %s time=%" PRIi64 " bio=" ptr_format "\n",
            mode,
            (int64_t)time(NULL),
            (ptr_type)bio);
    fflush(biofp);
