 ...
 */
static int
sitnl_send(struct nlmsghdr *payload,
           pid_t peer,
           unsigned int groups,
...
static int
sitnl_route_best_gw(sa_family_t af_family,
                    const inet_address_t *dst,
                    void *best_gw,
                    char *best_iface)
{
...
int
net_route_v6_best_gw(openvpn_net_ctx_t *ctx,
                     ...
                     char *best_iface)
{
    ...
    msg(D_ROUTE,
        "%s result: via %s dev %s",
        __func__,
        inet_ntop(AF_INET6, best_gw, buf, sizeof(buf)),
        best_iface);
...
/* used by iproute2 implementation too */
int
net_route_v4_best_gw(openvpn_net_ctx_t *ctx,
                     const in_addr_t *dst,
                     in_addr_t *best_gw,
                     char *best_iface)
{
    ...
    msg(D_ROUTE,
        "%s result: via %s dev %s",
        __func__,
        inet_ntop(AF_INET, best_gw, buf, sizeof(buf)),
        best_iface);
...
static int
sitnl_addr_ptp_add(sa_family_t af_family,
                   const char *iface,
                   const inet_address_t *local,
                   const inet_address_t *remote)
{
    ...
    return sitnl_addr_set(
        RTM_NEWADDR, NLM_F_CREATE | NLM_F_REPLACE, ifindex, af_family, local, remote, 0);
}
...
sitnl_addr_add(sa_family_t af_family, const char *iface, const inet_address_t *addr, int prefixlen)
{
    ...
    return sitnl_addr_set(
        RTM_NEWADDR, NLM_F_CREATE | NLM_F_REPLACE, ifindex, af_family, addr, NULL, prefixlen);
}
...
net_addr_v4_add(openvpn_net_ctx_t *ctx, const char *iface, const in_addr_t *addr, int prefixlen)
{
    ...
    msg(M_INFO,
        "%s: %s/%d dev %s",
        __func__,
        inet_ntop(AF_INET, &addr_v4.ipv4, buf, sizeof(buf)),
        prefixlen,
        iface);
...
int
net_addr_v6_add(openvpn_net_ctx_t *ctx,
                const char *iface,
                ...
                int prefixlen)
{
    ...
    msg(M_INFO,
        "%s: %s/%d dev %s",
        __func__,
        inet_ntop(AF_INET6, &addr_v6.ipv6, buf, sizeof(buf)),
        prefixlen,
        iface);
...
int
net_addr_v6_del(openvpn_net_ctx_t *ctx,
                const char *iface,
                const struct in6_addr *addr,
                int prefixlen)
{
...
int
net_addr_ptp_v4_add(openvpn_net_ctx_t *ctx,
                    const char *iface,
                    ...
                    const in_addr_t *remote)
{
    ...
    msg(M_INFO,
        ...
        inet_ntop(AF_INET, &remote_v4.ipv4, buf2, sizeof(buf2)),
        iface);
...
int
net_addr_ptp_v4_del(openvpn_net_ctx_t *ctx,
                    const char *iface,
                    const in_addr_t *local,
                    const in_addr_t *remote)
{
    ...
    msg(M_INFO,
        "%s: %s dev %s",
        __func__,
        inet_ntop(AF_INET, &local_v4.ipv4, buf, sizeof(buf)),
        iface);
...
static int
sitnl_route_add(const char *iface,
                sa_family_t af_family,
                const void *dst,
                int prefixlen,
                const void *gw,
...
int
net_route_v4_add(openvpn_net_ctx_t *ctx,
                 const in_addr_t *dst,
                 int prefixlen,
                 const in_addr_t *gw,
                 const char *iface,
                 ...
                 int metric)
{
    ...
    msg(D_ROUTE,
        "%s: %s/%d via %s dev %s table %d metric %d",
        __func__,
...
int
net_route_v6_add(openvpn_net_ctx_t *ctx,
                 const struct in6_addr *dst,
                 int prefixlen,
...
static int
sitnl_route_del(const char *iface,
                sa_family_t af_family,
                inet_address_t *dst,
                int prefixlen,
                inet_address_t *gw,
...
int
net_route_v4_del(openvpn_net_ctx_t *ctx,
                 const in_addr_t *dst,
                 int prefixlen,
                 const in_addr_t *gw,
                 const char *iface,
...
int
net_route_v6_del(openvpn_net_ctx_t *ctx,
                 const struct in6_addr *dst,
                 int prefixlen,
                 const struct in6_addr *gw,
                 const char *iface,
...
static int
sitnl_parse_rtattr_flags(
    struct rtattr *tb[], int max, struct rtattr *rta, int len, unsigned short flags)
{
    unsigned short type;
