...
verify_callback(void *session_obj, mbedtls_x509_crt *cert, int cert_depth, uint32_t *flags)
{
    ...
    if (*flags != 0)
    {
        ...
        if (ret <= 0
            && !snprintf(
                errstr, sizeof(errstr), "Could not retrieve error string, flags=%" PRIx32, *flags))
        {
            errstr[0] = '\0';
        }
        else
        ...
        {
            msg(D_TLS_ERRORS,
                "VERIFY ERROR: depth=%d, subject=%s, serial=%s: %s",
                cert_depth,
                subject,
...
static void
do_setenv_name(struct env_set *es,
...
void
x509_track_add(const struct x509_track **ll_head,
               const char *name,
               int msglevel,
               struct gc_arena *gc)
{
...
void
x509_setenv_track(const struct x509_track *xt,
                  struct env_set *es,
                  const int depth,
...
x509_verify_cert_eku(mbedtls_x509_crt *cert, const char *const expected_oid)
{
    ...
    else
    {
        ...
        while (oid_seq != NULL)
        {
            ...
            {
                msg(D_HANDSHAKE,
                    "++ Certificate has EKU (str) %s, expects %s",
                    oid_str,
                    expected_oid);
            ...
            {
                msg(D_HANDSHAKE,
                    "++ Certificate has EKU (oid) %s, expects %s",
                    oid_num_str,
                    expected_oid);
