...
do_address_service(const bool add, const short family, const struct tuntap *tt)
{
    ...
    address_message_t addr = {
        .header = { (add ? msg_add_address : msg_del_address), sizeof(address_message_t), 0 },
        .family = family,
...
do_wins_service(bool add, const struct tuntap *tt)
{
    ...
    wins_cfg_message_t wins = {
        .header = { (add ? msg_add_wins_cfg : msg_del_wins_cfg), sizeof(wins_cfg_message_t), 0 },
        .iface = { .index = tt->adapter_index, .name = "" },
        .addr_len = addr_len
    };
...
do_set_mtu_service(const struct tuntap *tt, const short family, const int mtu)
{
    ...
    {
        msg(M_INFO,
            "%s MTU set to %d on interface %d using service",
            family_name,
            mtu,
            mtu_msg.iface.index);
        ret = true;
...
static void solaris_error_close(struct tuntap *tt,
...
static void
check_addr_clash(
    const char *name, int type, in_addr_t public, in_addr_t local, in_addr_t remote_netmask)
{
    struct gc_arena gc = gc_new();
#if 0
...
init_tun_post(struct tuntap *tt, const struct frame *frame, const struct tuntap_options *options)
{
    ...
    {
        tt->wintun_send_ring_handle = CreateFileMapping(
            INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, sizeof(struct tun_ring), NULL);
        tt->wintun_receive_ring_handle = CreateFileMapping(
            INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, sizeof(struct tun_ring), NULL);
 ...
 */
static void
do_ifconfig_ipv6(struct tuntap *tt,
                 const char *ifname,
                 int tun_mtu,
                 const struct env_set *es,
                 openvpn_net_ctx_t *ctx)
{
    ...
    if (tt->type == DEV_TYPE_TUN)
    {
        ...
        argv_printf(&argv,
                    "%s %s inet6 plumb %s/%d %s mtu %d up",
                    IFCONFIG_PATH,
                    ifname,
                    ifconfig_ipv6_local,
                    tt->netbits_ipv6,
 ...
 */
static void
do_ifconfig_ipv4(struct tuntap *tt,
                 const char *ifname,
                 int tun_mtu,
                 const struct env_set *es,
                 openvpn_net_ctx_t *ctx)
...
{
    msg(D_LOW,
        "do_ifconfig, ipv4=%d, ipv6=%d",
        tt->did_ifconfig_setup,
    ...
    if (management)
    {
        management_set_state(
            management, OPENVPN_STATE_ASSIGN_IP, NULL, &tt->local, &tt->local_ipv6, NULL, NULL);
    }
#endif
...
#if defined(TARGET_LINUX) || defined(TARGET_FREEBSD)
static void
open_tun_dco_generic(const char *dev,
                     const char *dev_type,
                     struct tuntap *tt,
                     openvpn_net_ctx_t *ctx)
{
...
#define ANDROID_TUNNAME "vpnservice-tun"
    ...
    for (int i = 0; i < tt->options.dns6_len; i++)
    {
        management_android_control(
            management, "DNS6SERVER", print_in6_addr(tt->options.dns6[i], 0, &gc));
    }

    for (int i = 0; i < tt->options.dns_len; i++)
    {
        management_android_control(
            management, "DNSSERVER", print_in_addr_t(tt->options.dns[i], 0, &gc));
    }
...
static void
solaris_error_close(struct tuntap *tt,
                    const struct env_set *es,
                    const char *actual,
                    bool unplumb_inet6)
{
...
get_device_instance_id_interface(struct gc_arena *gc)
{
    ...
    for (DWORD i = 0;; ++i)
    {
        ...
        dev_key = SetupDiOpenDevRegKey(
            dev_info_set, &device_info_data, DICS_FLAG_GLOBAL, 0, DIREG_DRV, KEY_QUERY_VALUE);
        ...
        status = RegQueryValueEx(
            dev_key, net_cfg_instance_id_string, NULL, &data_type, net_cfg_instance_id, &len);
        ...
        res = SetupDiGetDeviceInstanceId(
            dev_info_set, &device_info_data, device_instance_id, len, &len);
        if (!res)
        {
            goto next;
        }
        ...
        while (strlen(dev_if) > 0)
        {
            ...
            msg(D_TAP_WIN_DEBUG,
                "NetCfgInstanceId: %s, Device Interface: %s",
                dev_iif->net_cfg_instance_id,
...
get_tap_reg(struct gc_arena *gc)
{
    ...
    while (true)
    {
        ...
        else
        {
            ...
            status = RegQueryValueEx(
                unit_key, component_id_string, NULL, &data_type, (LPBYTE)component_id, &len);
...
get_panel_reg(struct gc_arena *gc)
{
    ...
    status = RegOpenKeyEx(
        HKEY_LOCAL_MACHINE, NETWORK_CONNECTIONS_KEY, 0, KEY_READ, &network_connections_key);
    ...
    while (true)
    {
        ...
        else
        {
            ...
            status = RegQueryValueExW(
                connection_key, name_string, NULL, &name_type, (LPBYTE)name_data, &len);

            if (status != ERROR_SUCCESS || name_type != REG_SZ)
            {
                dmsg(D_REGISTRY,
                     "Error opening registry key: %s\\%s\\%ls",
                     NETWORK_CONNECTIONS_KEY,
                     connection_string,
                     name_string);
            }
...
static const struct tap_reg *
get_adapter_by_name(const char *name,
...
tap_allow_nonadmin_access(const char *dev_node)
{
    ...
    if (dev_node)
    {
        ...
        device_guid = get_device_guid(
            dev_node, actual_buffer, sizeof(actual_buffer), NULL, tap_reg, panel_reg, &gc);
...
netsh_set_dns6_servers(const struct in6_addr *addr_list, const int addr_len, DWORD adapter_index)
{
    ...
    argv_printf(&argv,
                "%s%s interface ipv6 delete dns %lu all",
                get_win_sys_path(),
                NETSH_PATH_SUFFIX,
                adapter_index);
                       ...
                       const bool test_first)
{
    ...
    {
        argv_printf(&argv,
                    "%s%s interface ip delete %s %lu all",
                    get_win_sys_path(),
                    NETSH_PATH_SUFFIX,
                    type,
                    adapter_index);
               ...
               const unsigned int flags)
{
    ...
    if (flags & NI_IP_NETMASK)
    {
        ...
        else
        {
            ...
            argv_printf(&argv,
                        "%s%s interface ip set address %lu static %s %s",
                        get_win_sys_path(),
                        NETSH_PATH_SUFFIX,
                        adapter_index,
                        print_in_addr_t(ip, 0, &gc),
...
windows_set_mtu(const int iface_index, const short family, const int mtu)
{
    ...
    {
        msg(M_INFO,
            "%s MTU set to %d on interface %d using SetIpInterfaceEntry()",
            family_name,
            mtu,
            iface_index);
    }
...
netsh_get_id(const char *dev_node, struct gc_arena *gc)
{
    ...
    {
        guid = get_unspecified_device_guid(
            0, BPTR(&actual), BCAP(&actual), tap_reg, panel_reg, NULL, gc);

        if (get_unspecified_device_guid(1,
                                        NULL,
                                        0,
                                        tap_reg,
                                        panel_reg,
...
write_dhcp_u32_array(
    struct buffer *buf, const int type, const uint32_t *data, const unsigned int len, bool *error)
{
    if (len > 0)
    {
        int i;
...
write_dhcp_search_str(
    struct buffer *buf, const int type, const char *const *str_array, int array_len, bool *error)
{
    char tmp_buf[256];
    int i;
    int len = 0;
...
service_register_ring_buffers(const struct tuntap *tt)
{
    ...
    register_ring_buffers_message_t msg = {
        .header = { msg_register_ring_buffers, sizeof(register_ring_buffers_message_t), 0 },
        .device = tt->hand,
...
tuntap_get_mtu(struct tuntap *tt)
{
    ...
    DWORD len;
    if (DeviceIoControl(
            tt->hand, TAP_WIN_IOCTL_GET_MTU, &mtu, sizeof(mtu), &mtu, sizeof(mtu), &len, NULL))
    {
    ...
    if (tt->backend_driver == WINDOWS_DRIVER_TAP_WINDOWS6 && index != TUN_ADAPTER_INDEX_INVALID)
    {
        ...
        if (tt->options.msg_channel)
        {
            ...
            flush_neighbors_message_t msg = {
                .header = { msg_flush_neighbors, sizeof(flush_neighbors_message_t), 0 },
                .family = AF_INET,
...
wintun_register_ring_buffer(struct tuntap *tt, const char *device_guid)
{
    ...
    tt->wintun_send_ring = (struct tun_ring *)MapViewOfFile(
        tt->wintun_send_ring_handle, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(struct tun_ring));

    tt->wintun_receive_ring = (struct tun_ring *)MapViewOfFile(
        tt->wintun_receive_ring_handle, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(struct tun_ring));
...
tuntap_set_ptp(const struct tuntap *tt)
{
    ...
    if (!tt->did_ifconfig_setup || tt->topology == TOP_SUBNET)
    {
        ...
        status = DeviceIoControl(
            tt->hand, TAP_WIN_IOCTL_CONFIG_TUN, ep, sizeof(ep), ep, sizeof(ep), &len, NULL);
        ...
        {
            msg(status ? M_INFO : M_FATAL,
                "Set TAP-Windows TUN with fake IPv4 [%s]",
                status ? "SUCCEEDED" : "FAILED");
        }
    }
    else
...
tuntap_dhcp_mask(const struct tuntap *tt, const char *device_guid)
{
    ...
    if (!DeviceIoControl(
            tt->hand, TAP_WIN_IOCTL_CONFIG_DHCP_MASQ, ep, sizeof(ep), ep, sizeof(ep), &len, NULL))
    {
        msg(M_FATAL,
...
static bool
tun_try_open_device(struct tuntap *tt,
                    ...
                    const struct device_instance_id_interface *device_instance_id_interface)
{
    ...
    if (tt->backend_driver == WINDOWS_DRIVER_WINTUN || tt->backend_driver == DRIVER_DCO)
    {
    ...
    {
        msg(D_TUNTAP_INFO | M_ERRNO,
            "CreateFile failed on %s device: %s",
...
void
tun_open_device(struct tuntap *tt,
                const char *dev_node,
                const char **device_guid,
                struct gc_arena *gc)
{
        ...
        *device_guid = get_device_guid(dev_node,
        ...
        if (tt->backend_driver != windows_driver)
        {
            msg(M_FATAL,
                ...
                dev_node,
                print_tun_backend_driver(windows_driver),
                print_tun_backend_driver(tt->backend_driver));
        }
