...
key_state_export_keying_material(
    struct tls_session *session, const char *label, size_t label_size, void *ekm, size_t ekm_size)
{
...
int
__wrap_parse_line(const char *line,
                  char **p,
                  const int n,
                  const char *file,
                  const int line_num,
...
test_tls_crypt_secure_reneg_key(void **state)
{
    ...
    memset(&session.tls_wrap.original_wrap_keydata.keys,
           0x00,
...
tls_crypt_loopback_max_len(void **state)
{
    ...
    buf_clear(&ctx->source);
    assert_non_null(buf_write_alloc(
        &ctx->source, TESTBUF_SIZE - BLEN(&ctx->ciphertext) - tls_crypt_buf_overhead()));
...
tls_crypt_fail_msg_too_long(void **state)
{
    ...
    buf_clear(&ctx->source);
    assert_non_null(buf_write_alloc(
        &ctx->source, TESTBUF_SIZE - BLEN(&ctx->ciphertext) - tls_crypt_buf_overhead() + 1));
...
test_tls_crypt_v2_setup(void **state)
{
    ...
    *state = ctx;
    ...
    init_key_ctx_bi(&ctx->server_keys,
                    &ctx->server_key2,
...
tls_crypt_v2_wrap_unwrap_no_metadata(void **state)
{
    ...
    assert_true(tls_crypt_v2_unwrap_client_key(
        &unwrapped_client_key2, &unwrap_metadata, wrapped_client_key, &ctx->server_keys.decrypt));

    assert_true(0
                == memcmp(ctx->client_key2.keys,
                          unwrapped_client_key2.keys,
...
tls_crypt_v2_wrap_unwrap_max_metadata(void **state)
{
    ...
    assert_true(tls_crypt_v2_wrap_client_key(
        &ctx->wkc, &ctx->client_key2, &ctx->metadata, &ctx->server_keys.encrypt, &ctx->gc));

    struct buffer unwrap_metadata = alloc_buf_gc(TLS_CRYPT_V2_MAX_METADATA_LEN, &ctx->gc);
    struct key2 unwrapped_client_key2 = { 0 };
    assert_true(tls_crypt_v2_unwrap_client_key(
        &unwrapped_client_key2, &unwrap_metadata, ctx->wkc, &ctx->server_keys.decrypt));
...
tls_crypt_v2_wrap_too_long_metadata(void **state)
{
    ...
    assert_false(tls_crypt_v2_wrap_client_key(
        &ctx->wkc, &ctx->client_key2, &ctx->metadata, &ctx->server_keys.encrypt, &ctx->gc));
}
...
tls_crypt_v2_wrap_unwrap_wrong_key(void **state)
{
    ...
    struct key2 unwrapped_client_key2 = { 0 };
    assert_false(tls_crypt_v2_unwrap_client_key(
        &unwrapped_client_key2, &ctx->unwrapped_metadata, ctx->wkc, &ctx->server_keys.decrypt));
...
tls_crypt_v2_wrap_unwrap_dst_too_small(void **state)
{
    ...
    assert_false(tls_crypt_v2_unwrap_client_key(
        &unwrapped_client_key2, &unwrapped_metadata, ctx->wkc, &ctx->server_keys.decrypt));
...
test_tls_crypt_v2_write_client_key_file_metadata(void **state)
{
    ...
    expect_string(__wrap_buffer_write_file, filename, filename);
    expect_memory(
        __wrap_buffer_write_file, pem, test_client_key_metadata, strlen(test_client_key_metadata));
    will_return(__wrap_buffer_write_file, true);
...
main(void)
{
    ...
    const struct CMUnitTest tests[] = {
        cmocka_unit_test_setup_teardown(
            tls_crypt_loopback, test_tls_crypt_setup, test_tls_crypt_teardown),
        cmocka_unit_test_setup_teardown(
            tls_crypt_loopback_zero_len, test_tls_crypt_setup, test_tls_crypt_teardown),
        cmocka_unit_test_setup_teardown(
            tls_crypt_loopback_max_len, test_tls_crypt_setup, test_tls_crypt_teardown),
        cmocka_unit_test_setup_teardown(
            tls_crypt_fail_msg_too_long, test_tls_crypt_setup, test_tls_crypt_teardown),
        cmocka_unit_test_setup_teardown(
            tls_crypt_fail_invalid_key, test_tls_crypt_setup, test_tls_crypt_teardown),
        cmocka_unit_test_setup_teardown(
            tls_crypt_fail_replay, test_tls_crypt_setup, test_tls_crypt_teardown),
        cmocka_unit_test_setup_teardown(
            tls_crypt_ignore_replay, test_tls_crypt_setup, test_tls_crypt_teardown),
        ...
        cmocka_unit_test_setup_teardown(tls_crypt_v2_wrap_unwrap_dst_too_small,
                                        ...
                                        test_tls_crypt_v2_teardown),
        cmocka_unit_test_setup_teardown(
            test_tls_crypt_secure_reneg_key, test_tls_crypt_setup, test_tls_crypt_teardown),
